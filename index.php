
<?php
 // Session
        session_start();
        include './connect/connect.php';
        include './fonction/requeteJournal.php';
        if(!empty($_SESSION['idu'])){
            header('location:admin/index.php');
        }
        if(isset($_POST['connexion'])){
        $req=$bdd->query('select * from utilisateur where login="'.$_POST['login'].'" and password ="'.$_POST['password'].'"');
        if($ligne=$req->fetch()){
            $_SESSION['idu']=$ligne['idu'];
            $libelle="connexion";
            insererJournal($libelle, $ligne['idu']);
            if($ligne['profil']=="Administateur"){
               header('location:admin/index.php');
            }
            else {
               header('location:couturie/index.php');
            }
            }
        else {
            $_POST['incorrect']='incorrect';
        }
        }
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="">
    <meta name="author" content="">
    <title>Connexion</title>
    <link rel="stylesheet" href="assets/css/default-css.css">
    <link href="theme/all.min.css" rel="stylesheet" type="text/css">
    <link
        href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
        rel="stylesheet">

    <!-- Custom styles for this template-->
    <link href="theme/sb-admin-2.min.css" rel="stylesheet">

</head>

<body >
  <div id="preloader">
        <div class="loader"></div>
    </div>
    <div class="container">

        <!-- Outer Row -->
        <div class="row justify-content-center" style="margin-top: 5%">

            <div class="col-xl-10 col-lg-12 col-md-9">

                <div class="card o-hidden border-0 shadow-lg my-5">
                    <div class="card-body p-0">
                        <!-- Nested Row within Card Body -->
                        <div class="row">
                            <div class="col-lg-6 d-none d-lg-block " style="background-image:url(icon/login.jpg);"></div>
                            <div class="col-lg-6">
                                <div class="p-5">
                                    <div class="text-center">
                                        <h1 class="h4 text-gray-900 mb-4">Sumar Creation Couture</h1>
                                    </div>
                                    <form class="user" action="index.php" method="post">
                                        <input type="hidden" name="incorrect">
                                        <div class="form-group">
                                            <input type="text" class="form-control form-control-user"
                                                id="exampleInputEmail" aria-describedby="emailHelp"
                                                placeholder="login exemple: bfofana" name="login" required="" 
                                                value="<?php if(isset($_POST['incorrect'])){    echo ''.$_POST['login'];  } ?>">
                                        </div>
                                        <div class="form-group">
                                            <input type="password" class="form-control form-control-user"
                                                   id="password" placeholder="Mot de passe" name="password" required=""
                                                   value="<?php if(isset($_POST['incorrect'])){    echo ''.$_POST['password'];  } ?>">
                                        </div>
                                        <div class="form-group">
                                            <div class="custom-control custom-checkbox small">
                                                <input type="checkbox" class="custom-control-input" id="customCheck" onclick="afficher()">
                                                <label class="custom-control-label" for="customCheck">afficher mot de passe
                                                    </label>
                                            </div>
                                        </div>
                                        <?php
                                        if(isset($_POST['incorrect'])){
                                            echo ' <div class="alert alert-danger" role="alert">
                                            Login ou mot de passe incorrect
                                        </div>';
                                        }
                                        ?>
                                        <input type="submit" name="connexion" class="btn btn-primary btn-user btn-block" value="Connexion">
                                        <hr>
                                        <script>
                                          function afficher(){
                                              var p=document.getElementById("password");
                                              if(password.type==="password"){
                                                  p.type="text";
                                              }
                                              else {
                                                  p.type="password";
                                              }
                                          }
                                        </script> 
                                    </form>
                                    
                                    <div class="text-center">
                                        <a class="small" href="password.php">Mot de passe oublié</a>
                                    </div>
                                    
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

        </div>

    </div>

    <!-- Bootstrap core JavaScript-->
    <script src="theme/jquery.min.js"></script>
    <script src="theme/bootstrap.bundle.min.js"></script>

    <!-- Core plugin JavaScript-->
    <script src="theme/jquery.easing.min.js"></script>

    <!-- Custom scripts for all pages-->
    <script src="theme/sb-admin-2.min.js"></script>
<script src="assets/js/scripts.js"></script>
</body>

</html>